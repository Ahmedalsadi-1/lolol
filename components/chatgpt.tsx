...
export function ChatGPTBlock() {
  return (
    <TooltipProvider>
      <div className="dark bg-background min-h-screen w-full flex items-center justify-center p-4">
        <PromptForm />
      </div>
    </TooltipProvider>
  )
      ...
      <Field>
        ...
        <InputGroup>
          ...
          <InputGroupAddon align="block-end">
            ...
            {/* Model Selector */}
            <DropdownMenu>
              <DropdownMenuTrigger render={<InputGroupButton variant="ghost" size="sm" className="rounded-4xl gap-1 px-2 text-muted-foreground">GPT-5.1
                <ChevronDownIcon className="size-3" /></InputGroupButton>} />
            ...
            <Tooltip>
              <TooltipTrigger render={<InputGroupButton variant="ghost" size="icon-sm" onClick={() => setDictateEnabled(!dictateEnabled)} className="ml-auto rounded-4xl"><AudioLinesIcon
                /></InputGroupButton>} />
...
function ModelSelector() {
  ...
  return (
    <Example title="Model Selector">
      <DropdownMenu>
        <DropdownMenuTrigger render={<Button variant="ghost" className="gap-2">ChatGPT 5.1
          <ChevronDownIcon className="text-muted-foreground size-4" /></Button>} />
        <DropdownMenuContent className="w-60" align="start">
          <DropdownMenuGroup>
            <DropdownMenuLabel className="text-muted-foreground text-xs font-normal">
              GPT-5.1
...
function GroupChatDialog() {
  return (
    <Example title="Group Chat Dialog" className="items-center justify-center">
      <AlertDialog>
        <AlertDialogTrigger render={<Button>Start Group Chat</Button>} />
        <AlertDialogContent>
          <AlertDialogHeader>
    ...
    <Example title="Create Project" className="items-center justify-center">
      <Card className="w-full max-w-sm">
        <CardHeader>
          ...
          <CardAction>
            <DropdownMenu>
              <DropdownMenuTrigger render={<Button variant="ghost" size="icon"><SettingsIcon
                /><span className="sr-only">Memory</span></Button>} />
          ...
          <FieldGroup>
            <Field>
              ...
              <InputGroup>
                ...
                <InputGroupAddon>
                  <Popover>
                    <PopoverTrigger render={<InputGroupButton variant="ghost" size="icon-xs"><FolderIcon style={
                                                                    { "--color": selectedColor } as React.CSSProperties
                                                                  } className="text-(--color)" /></InputGroupButton>} />
              ...
              <FieldDescription className="flex flex-wrap gap-2">
                {categories.map((category) => (
                  <Badge key={category.id} variant={
                      selectedCategory === category.id ? "default" : "outline"
                    } data-checked={selectedCategory === category.id} render={<button onClick={() => {
                          setSelectedCategory(
                            selectedCategory === category.id
                              ? null
                              : category.id
                          )
                        }}><CircleCheckIcon data-icon="inline-start" className="hidden group-data-[checked=true]/badge:inline" />{category.label}</button>} />
                ))}
              </FieldDescription>
            </Field>
            <Field>
